...
TxtSortingLine::TxtSortingLine(FISH_X1_TRANSFER* pTArea, ft::TxtMqttFactoryClient* mqttclient)
	: TxtSimulationModel(pTArea, mqttclient),
	currentState(__NO_STATE), newState(__NO_STATE),
	convBelt(pTArea, 0), chEW(4), chER(5), chEB(6), chComp(7),
...
int TxtSortingLine::readColorValue()
{
	...
	assert(pTArea);
	lastColorValue = pTArea->ftX1in.uni[1];
...
void TxtSortingLine::ejectWhite()
{
	...
	assert(pTArea);
	pTArea->ftX1out.duty[chEW] = 512;
	std::this_thread::sleep_for(std::chrono::milliseconds(500));
	pTArea->ftX1out.duty[chEW] = 0;
	setCompressor(false);
}
...
void TxtSortingLine::ejectRed()
{
	...
	assert(pTArea);
	pTArea->ftX1out.duty[chER] = 512;
	std::this_thread::sleep_for(std::chrono::milliseconds(500));
	pTArea->ftX1out.duty[chER] = 0;
	setCompressor(false);
}
...
void TxtSortingLine::ejectBlue()
{
	...
	assert(pTArea);
	pTArea->ftX1out.duty[chEB] = 512;
	std::this_thread::sleep_for(std::chrono::milliseconds(500));
	pTArea->ftX1out.duty[chEB] = 0;
	setCompressor(false);
}
...
void TxtSortingLine::configInputs()
{
	...
	assert(pTArea);
	//isStartColorSensor
	pTArea->ftX1config.uni[0].mode = MODE_R; // Digital Switch with PullUp resistor
	pTArea->ftX1config.uni[0].digital = 1;
	//isStartEjection
	pTArea->ftX1config.uni[2].mode = MODE_R; // Digital Switch with PullUp resistor
	...
	//isBlue
	pTArea->ftX1config.uni[7].mode = MODE_R; // Digital Switch with PullUp resistor
	pTArea->ftX1config.uni[7].digital = 1;
	//ColorSensor
	pTArea->ftX1config.uni[1].mode = MODE_U; // Analog Voltage
